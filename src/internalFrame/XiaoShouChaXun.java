...
public class XiaoShouChaXun extends JInternalFrame{
	...
	public XiaoShouChaXun() {
		...
		condition.setModel(new DefaultComboBoxModel(new String[] {"客户全称", "商品名称"}));
		setupComponet(condition, 1, 0, 1, 30, true);

		operation = new JComboBox();
		operation.setModel(new DefaultComboBoxModel(new String[]{"精准查询", "模糊查询"}));
		...
		String[] tableHeads = new String[]{"销售号码", "商品编号", "商品名称", "规格", "单价",
				"数量", "金额", "客户全称", "销售日期", "操作员", "经手人", "结算方式","备注"};
	...
	class ShowAllActoinListener implements ActionListener {			// 显示全部按钮的动作监听器
		public void actionPerformed(final ActionEvent e) {
			content.setText("");
			List list=ShuJuKu.findForList("select * from Sell_view");
	...
	class QueryActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = ShuJuKu.findForList("select * from Sell_view where "
					+ con
					+ opstr
					+ (oper == 0 ? "'"+cont+"'" : "'%" + cont + "%'")
					+ (selDate ? " and xsdate>'" + startDate.getText()+" 00:00:00"
