...
public class JiaGeTiaoZheng extends JInternalFrame {
	...
	private JLabel pinpai;
	private JLabel baoZhuang;
	private JLabel shengchanqiye;
	private JLabel gongysdw;
	public JiaGeTiaoZheng() {
		...
		addInternalFrameListener(new InternalFrameAdapter() {
			public void internalFrameActivated(final InternalFrameEvent e) {
				...
				List list = ShuJuKu.getKucunInfos();
		...
		pinpai = new JLabel();
		pinpai.setForeground(Color.BLUE);
		setupComponet(pinpai, 1, 1, 1, 1, true);
		setupComponet(new JLabel("供应商："), 2, 1, 1, 0, false);
		gongysdw = new JLabel();
		gongysdw.setForeground(Color.BLUE);
		setupComponet(gongysdw, 3, 1, 1, 1, true);
		...
		setupComponet(new JLabel("品　　牌："), 2, 2, 1, 0, false);
		shengchanqiye = new JLabel();
		shengchanqiye.setForeground(Color.BLUE);
		setupComponet(shengchanqiye, 3, 2, 1, 1, true);
	...
	class OkActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			int rs = ShuJuKu.updateKucunDj(kcInfo);
			if (rs > 0)
	...
	class ItemActionListener implements ItemListener {// 商品选择事件监听器
		public void itemStateChanged(final ItemEvent e) {
			...
			kcInfo = ShuJuKu.getKucun(item);
			if (kcInfo.getId() == null)
				return;
			int dj, sl;
			dj = kcInfo.getDj().intValue();
			sl = kcInfo.getKcsl().intValue();
			pinpai.setText(kcInfo.getPp());
			gongysdw.setText(kcInfo.getgysdw());
			baoZhuang.setText(kcInfo.getBz());
			shengchanqiye.setText(kcInfo.getScjy());
