package second.caoZuoYuanGuanLi;
...
public class TianJiaCaoZuoYuan extends JPanel {
	...
	public TianJiaCaoZuoYuan() {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				...
				Userlist user = ShuJuKu.getUser(tbName, password);
				if (user.getUsername() != null && !user.getUsername().isEmpty()) {
					JOptionPane.showMessageDialog(TianJiaCaoZuoYuan.this,
							"此登录名称已经存在");
					name.setFocusable(true);
					return;
				}
				if (name.getText() == null || name.getText().equals("")) {
					JOptionPane.showMessageDialog(TianJiaCaoZuoYuan.this, "用户名不能为空");
					name.setRequestFocusEnabled(true);
					return;
				}
				if (passwordField.getPassword().length == 0) {
					JOptionPane.showMessageDialog(TianJiaCaoZuoYuan.this, "密码不能为空");
				} else if (passwordField.getText().equals(
						passwordField_1.getText())) {
					Userlist tbUserlist = new Userlist();
					...
					ShuJuKu.addUser(tbUserlist);
					JOptionPane.showMessageDialog(TianJiaCaoZuoYuan.this, "操作员添加成功");
					clear();
				} else {
					JOptionPane
							.showMessageDialog(TianJiaCaoZuoYuan.this, "两次密码输入不相同");
				}
			}
		});
		button.setText("添加");
